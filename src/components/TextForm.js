  ...
  return (
    <>
      <div className='container' style={{color: props.mode === "dark"?"white":"black"}}>
          ...
          <button className="btn btn-primary mx-1 my-1" onClick={handleExtraSpaces}>Remove Exrta Spaces</button>
      </div>
      <div className="container my-3" style={{color: props.mode === "dark"?"white":"black"}}>
        <h3>Your Text Summary</h3>
        <p>{wordCount} words {charCount} characters</p>
        <p>{0.192*text.split(" ").length} seconds or {0.0032*text.split(" ").length} minutes taken to read</p>
        <h4>Preview</h4>
